...
agents_distance(Agent1, Agent2, Distance) :-
    ...
    % Calculate Manhattan distance and assign it to Distance.
...
number_of_agents(State, NumberOfAgents) :-
    % Extract the agent dictionary from the state.
    State = [Agents, _, _, _],
    % Retrieve all keys (agent IDs) and determine their count.
    dict_keys(Agents, Keys),
    length(Keys, NumberOfAgents).


% 3- value_of_farm(+State, -Value)
% Calculates the total value of the farm by summing the values of agents and objects.
value_of_farm(State, Value) :-
    % Extract agents and objects dictionaries from the state.
...
value_sum(_Id-Entity, Acc, NewAcc) :-
...
find_food_coordinates(State, AgentId, Coordinates) :-
    ...
    % Find coordinates where the agent can eat the object or other agent.
    findall([X, Y], (
        (
            % Check objects for consumable types.
        ...
        (
            % Check other agents for consumable types (for carnivores).
    ...
    % Ensure we found at least one consumable item.
    Coordinates \= [].


% 5- find_nearest_agent(+State, +AgentId, -Coordinates, -NearestAgent)
% Finds the nearest agent from a given agent.
find_nearest_agent(State, AgentId, Coordinates, NearestAgent) :-
    % Extract agents dictionary from the state.
    State = [Agents, _, _, _],
    get_dict(AgentId, Agents, Agent),
    get_dict(x, Agent, X),
    get_dict(y, Agent, Y),
    % Find all other agents and calculate distances.
    findall(Dist-AId, (
    ...
    % Sort distances and pick the nearest one.
    sort(Dists, [_MinDist-NearestId|_]),
...
% Finds the nearest food to a given agent and returns its type and distance.
...
find_nearest_food(State, AgentId, Coordinates, FoodType, Distance) :-
    ...
    get_dict(x, Agent, Ax),
    get_dict(y, Agent, Ay),
    % Use find_food_coordinates to get potential food locations.
    find_food_coordinates(State, AgentId, Foods),
    % Calculate distances to all foods and sort them.
    findall(Dist-[X, Y], (
        member([X, Y], Foods),
    ...
    Coordinates = [Fx, Fy],
    % Get the food object from its position to determine its type.
...
% Recursive helper that builds a list of actions leading to the target coordinates.
...
move_to_nearest_food(State, AgentId, ActionList, DepthLimit) :-
    find_nearest_food(State, AgentId, [X, Y], _FoodType, _Distance),
    move_to_coordinate(State, AgentId, X, Y, ActionList, DepthLimit),
    ActionList \= [].
...
consume_all_helper(_State, _, MovAcc, ValAcc, ChildAcc, MovAcc, ValAcc, ChildAcc, _, _).
